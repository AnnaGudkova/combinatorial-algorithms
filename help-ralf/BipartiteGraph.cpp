...
BipartiteGraph::BipartiteGraph(const std::vector< std::vector<int> > &link_list) {
    this->link_list = link_list;
    this->left_vertex_number  = link_list.size();
    this->rigth_vertex_number = link_list[0].size();
}
...
std::vector< std::vector<int> > BipartiteGraph::getLinkList() {
